...
const listChirpsFromUserID = `-- name: ListChirpsFromUserID :many
SELECT id, body, user_id, created_at, updated_at FROM chirps
WHERE id = $1
ORDER BY created_at ASC
`

func (q *Queries) ListChirpsFromUserID(ctx context.Context, id uuid.UUID) ([]Chirp, error) {
	rows, err := q.db.QueryContext(ctx, listChirpsFromUserID, id)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
